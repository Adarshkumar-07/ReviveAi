...
def simulate_recovery(tx_id, approved=False):
    ...
    with conn() as c:
        ...
        # Simulation outcome comes from the independent synthetic ground-truth
        # process, not from the model prediction. This avoids circular evaluation.
        outcome = 'recovered' if __import__('random').Random(sum(map(ord, tx_id))).random() < latent_probability(tx) else 'failed'
        c.execute(
