# ReviveAI

**AI Revenue Recovery Agent**  
**Recover the right revenue. Automatically. Safely. Measurably.**

**Team:** Cyber Sage  
**Track:** Track 03 — AI Revenue Recovery

ReviveAI is a merchant revenue-recovery command center for Razorpay AI Buildathon **Track 03 — AI Revenue Recovery**. It combines a trained recovery-probability model, optional OpenAI reasoning, deterministic financial policies, simulated execution, measurement and an audit trail.

## Team Cyber Sage

| Member | Role | Contribution |
|---|---|---|
| **Adarsh Kumar** | Backend & AI Integration | Flask backend, AI/LLM orchestration, policy-engine integration, API development, recovery workflow integration, testing and QA |
| **Rohan Sharma** | Frontend & Product | React/Vite dashboard, transaction views, Decision Trace UI, responsive design, user workflow and frontend testing |
| **Dhruv** | ML, Evaluation & Documentation | Recovery-probability ML model, synthetic dataset/evaluation, recovery metrics, audit/evaluation analysis, documentation and demo preparation |

## Problem

Every failed payment is not equally recoverable. Blind retries can waste attempts, increase customer friction and miss high-value opportunities. Merchants need to know **which revenue is worth pursuing, why it failed, what intervention is safest, and when to stop**.

## Solution

ReviveAI follows:

`DETECT → DIAGNOSE → PREDICT → DECIDE → VALIDATE → RECOVER → MEASURE → AUDIT`

The ML model estimates recovery probability. The LLM diagnoses the event and recommends an intervention. A deterministic policy engine validates or blocks that recommendation. Only approved actions reach the simulation layer.

## Demo

The dashboard is intentionally built as an operations product rather than a chatbot. It includes a live recovery queue, decision trace, policy status, recovery execution, batch evaluation and safety posture.

### 5-Minute Pitch Video

▶️ **[Watch the ReviveAI 5-minute pitch](https://drive.google.com/file/d/15jVq5fQhPeFukC_Zih6VVF0sKVV3RtdV/view?usp=drive_link)**

The video demonstrates the complete flow:

`Problem → Live Dashboard → AI Decision → Policy Gate → Simulated Recovery → Duplicate Protection → Human Approval → Stopping Rule → Architecture → Evaluation`

> **Demo environment:** test/simulation mode only. No real payment was charged.

## Screenshots

### Revenue Command Center

![ReviveAI Revenue Command Center](docs/screenshots/dashboard.png)

The main operations dashboard shows revenue at risk, ML-qualified opportunities, recovered revenue, recovery attempts and the live recovery queue.

### Decision Trace — Approved Recovery

![ReviveAI Decision Trace](docs/screenshots/decision-trace-approved.png)

The Decision Trace shows recovery probability, AI diagnosis, recommendation, deterministic policy outcome and the simulated recovery action.

### Safety Control — Stopping Rule

![ReviveAI Stopping Rule](docs/screenshots/stopping-rule.png)

When the retry limit is reached, the deterministic policy engine stops the recovery workflow and prevents execution.

### Safety Control — Duplicate Prevention

![ReviveAI Duplicate Prevention](docs/screenshots/duplicate-prevention.png)

A second execution attempt for the same recovery is blocked with **“Duplicate execution prevented.”**

> Screenshots show the deployed ReviveAI test/simulation environment. No real payments are charged.

Run locally:

```bash
pip install -r requirements.txt
python app.py
```

In another terminal:

```bash
cd frontend
npm install
npm run dev
```

### Optional OpenAI reasoning

Set the API key as an environment variable; never commit it to GitHub:

```bash
export OPENAI_API_KEY="your-key"
export OPENAI_MODEL="gpt-5.6-luna"
python app.py
```

If no key is configured, ReviveAI automatically uses a clearly labelled deterministic fallback, so the complete demo remains runnable without an API key.

## Architecture

```text
Merchant / Payment Events
          ↓
 Random Forest Risk Model
          ↓
      LLM Reasoning
   Diagnose + Recommend
          ↓
   Deterministic Policy
   ┌──────┴────────┐
   ↓               ↓
Approved       Approval/Stop
   ↓               ↓
Simulation     Human Gate
   └──────┬────────┘
          ↓
 Recovery Event + Audit
          ↓
 Revenue & Evaluation Metrics
```

**Core principle:** `LLM proposes → Policy Engine validates → Recovery executes → Audit measures`

## AI Decision Pipeline

1. **Detect** failed revenue events.
2. **Predict** recovery probability using Random Forest.
3. **Diagnose** the failure using OpenAI when configured, otherwise a deterministic fallback.
4. **Recommend** the safest next intervention from a constrained action set.
5. **Validate** the recommendation through deterministic policy.
6. **Execute** only in simulation/test mode.
7. **Measure** recovery and lift against an independent synthetic outcome process.
8. **Audit** every decision and execution event.

## Policy Engine

The policy layer is independent from AI and can override AI recommendations.

- Autonomous amount threshold: ₹10,000
- Retry limit: 2 prior attempts
- Minimum recovery probability: 35%
- Expired card → payment-method update
- Authentication required → re-authentication
- Insufficient funds → payment link / wait
- Unknown cause → human approval
- Action allowlist enforced
- Duplicate execution blocked
- No real payment is charged

## Dataset

The project uses reproducible **synthetic transaction data** containing amount, failure reason, payment method, attempts, customer age, historical success rate, prior failures, time since failure and subscription state.

The dataset is generated deterministically. Synthetic ground-truth recovery outcomes use a separate seeded process based on the transaction features; they are not sampled from the model prediction. This prevents circular evaluation.

## Evaluation Methodology

The Random Forest is evaluated using a strict split:

- 70% training
- 15% validation for threshold selection
- 15% held-out test set

The validation set selects the decision threshold using F1. The held-out test set is not used for tuning.

Revenue recovery is evaluated separately from classification. A strategy is compared with a simple bounded transient-failure retry baseline using the **same independent synthetic outcomes**. The evaluation therefore does not treat the model's own probability as its ground truth.

## Results

Results are generated by the repository's evaluation endpoint and dashboard. Metrics include:

- ROC-AUC
- Precision
- Recall
- F1
- Revenue at risk
- ML-qualified recoverable revenue
- Measured recovered revenue
- Baseline recovered revenue
- Recovery lift

**Measured synthetic evaluation:** **+43.6% recovery lift**, with **₹14,12,501 recovered vs ₹9,83,570 baseline**.

**Important:** the dataset is synthetic and execution is simulated/test-mode. These results demonstrate system behavior and evaluation methodology, not production financial performance.

## Failure Cases

ReviveAI intentionally does not force recovery when:

- the probability is below threshold;
- retry limits are reached;
- a payment method must be updated;
- customer authentication is required;
- the transaction exceeds the autonomous amount threshold for retries;
- the failure reason is ambiguous;
- an action is outside the allowlist;
- a duplicate recovery is detected.

## Safety & Guardrails

AI proposes; deterministic policy decides. Financial execution is never delegated directly to an LLM. Every decision records the probability, diagnosis, recommendation, final action and policy outcome. The demo is explicitly simulation/test mode and contains no real-money charging path.

## Setup

### Backend

```bash
pip install -r requirements.txt
python app.py
```

### Frontend

```bash
cd frontend
npm install
npm run dev
```

### Environment

```text
OPENAI_API_KEY=optional
OPENAI_MODEL=gpt-5.6-luna
```

`.env` and the local SQLite database are ignored by Git. Never place a real API key in source code, README files or committed configuration.

### Production-style single service

Render builds the Vite frontend and runs Flask/Gunicorn. The Flask app serves `frontend/dist` after the frontend build.

## API Documentation

| Method | Endpoint | Purpose |
|---|---|---|
| GET | `/api/health` | Health + mode |
| GET | `/api/metrics` | Recovery KPIs |
| GET | `/api/transactions` | Recovery queue |
| GET | `/api/transactions/<id>` | Transaction detail |
| POST | `/api/transactions/seed` | Reset synthetic dataset and all simulation state |
| POST | `/api/decide` | ML + LLM + policy decision |
| POST | `/api/recover` | Simulated/test execution |
| POST | `/api/simulate` | Run a clean fresh batch |
| GET | `/api/evaluation` | Evaluation metrics |
| GET | `/api/audit` | Audit events |

## Tests & CI

The repository includes regression tests for policy normalization, stopping rules, human-review gating, decision action propagation and simulation reset. GitHub Actions runs compilation, pytest and the Vite production build on pushes and pull requests.

## Future Work

- Razorpay test-mode integration
- Human approval queue UI
- Email/WhatsApp recovery simulations
- Multi-merchant policy profiles
- Real-time event ingestion
- Online model monitoring and drift detection

## Known Limitations

- Synthetic data does not represent production merchant distributions.
- LLM reasoning falls back to deterministic reasoning when no API key is configured or an LLM call fails.
- Recovery execution is simulated and cannot charge real money.
- Evaluation lift is a synthetic experiment, not a production guarantee.

## Tech Stack

- **Frontend:** React + Vite
- **Backend:** Python + Flask
- **AI:** OpenAI API with deterministic no-key fallback
- **ML:** scikit-learn Random Forest
- **Database:** SQLite
- **Async:** Celery/Redis intentionally deferred until core product stability
- **Deployment:** Render
- **Data:** Synthetic transaction dataset

## Buildathon Positioning

ReviveAI is designed around the Track 03 requirement to detect revenue at risk, choose the right intervention, execute bounded recovery workflows and demonstrate measurable recovered revenue. The key product idea is simple: **do not recover every failed payment — recover the right ones, and prove why.**

## License

MIT
